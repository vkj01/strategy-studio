...
branding.inject_css()
st.markdown("<style>.block-container { max-width: 950px; }</style>", unsafe_allow_html=True)

brain = ai_assistant.active_label("strong") if ai_assistant.llm_available() else "offline (add a key to .env)"
branding.hero("AI Assistant", "Ask in plain English -- I run real backtests, sweeps, and code. Brain: <b>%s</b>" % brain)
...
def cost_caption(cost):
    ...
    st.caption("💸 **Cost of this answer:** $%.4f  (~Rs %.2f)  ·  %s  ·  %d LLM call(s), "
               "%s in / %s out tokens  ·  %d backtest run(s)"
               % (usd, usd * _USD_INR, _model_short(cost.get("model")), cost.get("llm_calls", 0),
...
def _save_current():
    try:
        ...
        st.warning("Couldn't save this chat right now (%s). Your messages are still "
                   "on screen -- try again in a moment." % e)
...
with st.sidebar:
    ...
    try:
        ...
        st.warning("Past chats unavailable right now (%s)." % e)
        past = []
    if past:
        ...
        for c in past:
            if st.button(c["title"] or "chat", key="cv_" + c["id"], use_container_width=True):
                ...
                try:
                    ...
                    st.warning("Couldn't load that chat (%s)." % e)
                    st.session_state.assistant_chat = []
