"""
Short Futures -- sell the near-month NIFTY future (directional, intraday).

Profits when the index falls. Optional stop-loss / target / step-trailing-stop
(as % of the entry price) and entry/exit times. A single linear FUT leg -- no
strikes. StockMock's Futures segment, Sell side.
"""
...
META = {
    ...
    "name": "Short Futures",
    "instrument": "futures",
    "description": ("Sell the near-month NIFTY future. Directional bearish, intraday. "
...
def legs(ctx, params):
    return [{
        "type": "FUT", "action": "sell", "symbol": "NIFTY",
